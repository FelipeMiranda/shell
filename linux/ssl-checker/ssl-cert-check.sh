...
# Determine hostname from server used by MACS monitoring system.
HOSTNAME=$(hostname)
...
case $1 in
     ...
     -o)
       ...
       SUBJECT="Todos os certificados SSL estao validos [status=OK]"
       RESULT="${SUBJECT}"
...
EOM
     ...
     -c)
       ...
       SUBJECT="Existem certificados SSL expirados [status=CRITICAL]"
       RESULT="${SUBJECT}"
       STDOUT="$(cat ${STDOUT_TMP})"
...
####################################################
# Purpose: Sending events to MACS
# Arguments:
#   None
####################################################
URL_QUEBEC="http://quebec.api.srv.intranet/quebec-api/carrier"
...
usage()
{
    ...
    echo "  -F                : Send Fake OK Status to Quebec"
...
if [ ! -z "${CERT_TMP}" ] && [ ! -z "${ERROR_TMP}" ] && [ ! -z "${STDOUT_TMP}" ]
then
    touch ${CERT_TMP} ${ERROR_TMP} ${STDOUT_TMP}
else
...
then
    print_heading | tee -a ${STDOUT_TMP}
    ...
    done < ${SERVERFILE} | tee -a ${STDOUT_TMP}
...
rm -f ${CERT_TMP} ${ERROR_TMP} ${STDOUT_TMP}
